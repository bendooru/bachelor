...
Also \texttt{gfan.lib} \cite{gfanLib} % TODO!!
...
a means to formulate parallelizable workflows and consequently formulate our traversal
...
\dots, r$. This important fact ensures that, by successively computing neighboring
...
\begin{enumerate}[label=\arabic*.]
  \item If we already start out in the \emph{constant coefficient case}, that is, the
    ...
    $f \in I$ the initial form $\in_{\vec w}(f)$ will be just the minimal terms with
    ...
    \cite[Remark~2.5.11]{sturmMacTrop} on the connection between the general connection
    between Gröbner complexes and fans. Note that the current implementation in
    \texttt{tropicalNewton.lib} \cite{tropNewtLib} is designed in this way and handles
    only the constant coefficient case for initial ideals.
...
environment, along with the other important extensions: % TODO citations
\begin{description}[leftmargin=\parindent]
  \item[Time:] A traditional Petri net cannot model execution times of its transitions
    ...
    description of a realistic workflow. Timed Petri nets introduce them concept of time
  ...
  \item[Tokens and Data:] Until now all tokens in the classical Petri net held no
    ...
    amount of information that can be exchanged between independent processes. This leads
    to the notion of colored Petri nets: We extend the Petri net $(P,T,F)$ with a finite
    set $\Sigma$ of \emph{token color sets} or \emph{types} together with a \emph{color
    function} $C:P\to\Sigma$. In particular, all tokens in a given place have the same
    coloring.
...
<struct name="problem_data">
  ...
  \item[Conditions:] GPI-Space extends the firing process of transitions with conditions
    ...
    % TODO add some matching example
...
% TODO check position of condition, etc
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[node distance=1.4cm]
    \node [place] (in) {$i$};
    \node [transition] (plus) [right of=in] {$+$}
      edge [pre, bend left] (in)
      edge [post,bend right] (in);
    \node [place] (out) [right of=plus] {$o$}
      edge [pre] (plus);
    \node at (out) [above=0.4cm] {\footnotesize\texttt{\$\{o\}\,:geq:\,4}};
  \end{tikzpicture}
...
\begin{enumerate}[label=\arabic*.]
  \item We must first decide how to store and represent input and intermediate results.
    ...
    actual cone obtained from them to the data as well. Furthermore, we require the ambient
...
\cite[Sectiion~4.4]{reinboldGitFan} to traverse the GIT-fan.
\begin{itemize}
