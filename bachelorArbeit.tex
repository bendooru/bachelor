...
closed field $K$ with non-trivial valuation $v:K\to\setR \cup \{\infty\}$ and residue
field $\mathfrak K$ with uniformizing parameter $p$.
...
each set in the triangular composition:
...
algorithm is required in the rare case that a polynomials does not admit a unique Newton
...
% insert into previous section, the reference properly
This can then be used in Algorithm~??? (\cite[Algorithm~4.8]{compTropVar}) as a
replacement
