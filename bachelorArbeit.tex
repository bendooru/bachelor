...
tropical quadrics and cubics respectively. For instance, in figure~\ref{fig:tropLineQuad}
we see the tropical line $V(x \oplus y \oplus 0)$ on the left. The tropical quadric on
the right will be mentioned again later on. As a final note, since tropical division is
well-defined for non-infinite elements we may also consider tropical polynomials in
negative exponents later on.
...
% corr. definition of initial term

With this problem out of the way, we are now finally able to define tropical varieties,
the main object of study of this thesis:
...
\begin{example}
  ...
  depicted in Figure~\ref{fig:tropLineQuad} on the right.
\end{example}
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item On the practical side we may also start with an ideal $I \idealof \setC[\vec x]$
    and embed it into $\CCt[\vec x]$ so that our initial ideals behave analogously to
    Definition~\ref{def:initFormG}, thus yielding a polyhedral fan. The properties of the
    field $\CCt$ are then used in the methods described in section~\ref{sec:newtonMethod}.
    In fact, the current implementation in \texttt{tropicalNewton.lib} is designed in this
    way and handles only the \emph{constant coefficient case} for initial ideals. All
    other cases may then be transformed as in the first point.
\end{enumerate}
In addition to this, we are also allowed to only consider tropical varieties given by
homogeneous ideals. Fix an ideal $I\ideal K[x_1, \dots, x_n]$ and a weight $\vec w \in
...
While not quite as important to our purposes, we still want to look at how to
\enquote{intersect} to polyhedral fans. Given such fans $\mathcal F_1, \mathcal F_2$ we
define their \emph{common refinement} to be $\mathcal F_1 \wedge \mathcal F_2 := \{ C_1
\cap C_2 : C_1 \in \mathcal F_1, C_2 \in \mathcal F_2 \}$. This is again a fan and clearly
the support of $\mathcal F_1 \wedge \mathcal F_2$ is the intersection of the supports of
$\mathcal F_1$ and $\mathcal F_2$ respectively. Also, by considering representations of
the respective fans we obtain a representation of the common refinement in the following
way, and note that in practice we canonicalize the resulting intersection cones, allowing
us to easily check for duplicates:
...
% TODO citation
Another important step needed to compute neighbouring cones is to compute Gröbner bases in
these adjacent Gröbner cones. Generally speaking it would be possible to compute these
from scratch if we're just given out ideal $I$ and a weight $\vec w$ in the relative
interior of an adjacent cone. However, the following \emph{lift} algorithm which is a step
in the \emph{Gröbner walk} (cite sth. ???) greatly simplifies this procedure as we already
know the reduced Gröbner bases in the original Gröbner cone.
