...
counterpart. As usual, mathematicians are interested in studying concrete examples of
tropical varieties study -- for instance to gain a general intuition behind the
combinatorial structures or to find counter examples.
\todo{more on tropical geometry?}
...
examples that cannot be computed in reasonable time frames using single-threaded legacy
applications that are in use now. There are mainly two ways of tackling this issue:
...
\emph{multi-process systems} data is processed by different processes which not share
memory but instead communicate by message passing. This only provides coarse-grained
parallelism but can in theory scale to arbitrarily large computing configurations.

Our contribution in this thesis is the combination of the latter kind of paralellization
method mentioned above with existing methods to compute tropical varieties. Methods of
computing tropical varieties have been studied by Bogart, Jensen, Speyer, Sturmfels and
Thomas \cite{compTropVar} and further improved by Ren and Hofmann \cite{tropPointsLinks}.
These methods constitute the \enquote{lower} level of the final parallel implementation
and are implemented in the computer algebra system \textsc{Singular} \cite{Singular} which
is being developed at Technische Universität Kaiserslautern. In particular, tropical
computations are carried out by methods of the library \texttt{tropicalNewton.lib}
\cite{tropNewtLib} which relies on Gfan \cite{gfan} for polyhedral computations via the
interface library \texttt{gfan.lib} \cite{gfanLib}. Instances of \textsc{Singular}
processes are then managed by the workflow management system GPI-Space \cite{gpispace} at
the \enquote{upper} level. GPI-Space is developed by the High Performance Computations
(HPC) department at Fraunhofer Institut für Techno- und Wirtschaftsmathematik (ITWM).

Our parallelization solution is based on work by Christian Reinbold who used GPI-Space to
implement a parallel algorithm to compute GIT-fans \cite{reinboldGitFan}. This, in turn,
relies on foundational work done by Dr.~Lukas Ristau in creating interface code between
GPI-Space and \textsc{Singular}. As tropical varieties admit the structure of a polyhedral
fan as well, we were able to successfully modify Reinbold's algorithm to compute tropical
varieties instead. The development was coordinated in cooperation with the HPC department
of Fraunhofer ITWM, in particular with the support of Dr.~Mirko Rahn and Christian
Reinbold. Additionally, as a side effect we accelerated the development of
\texttt{puiseux.lib} \cite{puiseuxLib} lead by Santiago Laplagne on the one hand, which is
used as a fallback for method computing Puiseux expansions of roots of zero-dimensional
ideals. On the other hand during our work with \textsc{Singular} we were able to identify
several bugs -- mainly memory leaks -- which were promptly fixed. Using our parallel
algorithm we were able to compute the tropical Grassmannian $\mathcal G_{3,8}$ modulo
symmetry in its entirety, which was not possible with legacy methods until now. Using this
as benchmark, we also found that our algorithm scales favorably to large processor counts.
\todo{check for completion}
...
polyhedral convexes. Finally, we illustrate how to compute tropical varieties using
methods from \cite{compTropVar} which we then improve using new techniques from
\cite{tropPointsLinks}. In the third section we will introduce GPI-Space and Petri nets as
a means to formulate parallelizable workflows and subsequently translate our traversal
algorithm into such a Petri net. In the fourth section we introduce the tropical
...
\end{figure}
\todo{quadric not generic, add one?}
...
\todo{a \emph{bit} more introduction?}
...
= R_\nu/\mathfrak m_{R_\nu}$ of $R_\nu$.
\todo{do we even need the uniformizing parameter? (removed for now)}

Various fields with non-trivial valuation are suitable for computational purposes. The one
over which we will usually do our computations is the field of Puiseux series:
...
\todo{explain more or provide citation}
...
\todo{maybe restructure, provide citation}
...
elements of $I$ it is easy to see that $\Trop(I) = \Trop(I')$: \todo{can we just say
this?} If $\vec w \not\in
...
\begin{algo}[Lift]\
  ...
  \begin{algorithmic}[1]
    ...
    \todo{actually formulate the algorithm if possible}
  \end{algorithmic}
\end{algo}
...
details): \todo{might need other sources}
...
\begin{itemize}
  ...
  \item Finally, the transition $n$ consumes tokens from $c$ which hold Gröbner cone
...
\begin{itemize}
  \item Splitting the algorithm allows for a more fine-grained control over the job-sizes
    submitted to the run-time system. \todo{why is this good?}
...
\begin{itemize}
  ...
  \item The places resp.\ transitions $i_k, e_k, g_k$ for $k \in \{1, 2\}$ work
    analogously to the previous net. For $k=1$ we manage maximal Gröbner cones represented
    by their meta data, for $k=2$ we manage interior facet points together with their
    minimal representative with respect to $G$.
...
Grassmannians found in \cite[Section~8]{gathmAlgGeo}. \todo{provide other source?}
...
\todo{formalize this above?} The symmetry group $S_7$ acts naturally on the indices of the
...
translated into the slower \text{Singular} interpreter language\todo{talk earlier about
Singular features?}{} in the GPI-Space implementation.
...
\begin{table}[tbh]
  ...
  \begin{tabular}{r||r|r||r}
    ...
    32  &      -- &  39.756 & 0.623
  \end{tabular}
  \caption{Timings of computing $\mathcal G_{3,7}$}
  \label{tab:g37}
\end{table}
...
\todo{give reasons for efficiency drop}
...
This example is as interesting to us since it has already been studied quite thoroughly
and does not parallelize well beyond two compute nodes. Hence in the next step we want to
...
\todo{in progress (section useless?)}
