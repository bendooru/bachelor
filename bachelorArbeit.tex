...
\]
for all $\vec v$ some some appropriate $\epsilon>0$. Hence $\Trop(\initial_{\vec u}(I))$
...
on.
\todo{formulate entire (easy) Petri net here or above}
...
\begin{itemize}
  ...
  \item As first tests have shown using the built-in virtual memory layer of GPI-Space is
    ...
    provided by Fraunhofer ITWM this is achieved via \textsc{BeeGFS} \cite{bgfs}, which
    allows efficient parallel access to storage.
\end{itemize}
...
Another problem that might arise from this is the limited file length of most file
...
\todo{write stuff, include some real life code, termination logic?}
...
\begin{table}[tbh]
  ...
  \begin{tabular}{r|r||r|r}
    ...
    40 & 640 &  1830.740 & 0.791 \\ % FIXME watch out for bad data
...
possible explanations include memory and storage latencies on fewer machines.\todo{not
sure}{} Secondly, while we observe the expected degradation of parallelization efficiency
...
computation time and drop in efficiency. We can imagine two reasons for this:
\begin{itemize}
  \item As the sample size of runs is rather small, disturbances in the computing cluster
    can have significant impact to the average timings.
...
We can conclude that parallelizing algorithms for algebraic geometry is a worthwhile
endeavor and offers tremendous speed-up.
