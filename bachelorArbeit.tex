...
\begin{example}\
  \begin{enumerate}
    ...
    \item Consider the four quadrants
      ...
      C_3 \}$ which is again pure and complete. The set of quadrant is a representation of
      this fan.
  \end{enumerate}
\end{example}
...
$\initial_{\vec w}(f)$, so it cannot be a monomial. This argument works analogously in the
reverse direction, so the claimed equality is clear.
...
% TODO: define representation of Gröbner cone by std base pair here
...
in \ref{thmc:tropInt}.\ref{thmc:ii}. Note that, since the tropical curves are the supports
of tropical fans in this setting, they are necessarily collections of rays, hence the
representation as $\bigcup_{w \in W} w\cdot \setR_{\geq0} + C_0(I)$ is justified. This can
then be used in line \ref{alg:interiorPoints} of Algorithm~\ref{alg:neighbors} as a
replacement for the methods concerning prevarieties.
...
\begin{example}[Parallelisms in Petri Nets]
  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    \item Consider now the Petri net given by the graph depicted in
      Figure~\ref{fig:petri:taskpar}. If we are given the marking $M = \{ (i,1) \}$, then
...
As was hinted at above, Petri nets in the form given above, while offering exceptional
parallelization possibilities, are not yet suitable to model algorithms together with
the data they produce as they arise from real world applications.
...
conditions to properly stop the execution of the net.

However, before we can tackle this problem we must consider some implementation details
first that will be reflected in the description of our algorithm.
\begin{enumerate}[label=\arabic*.]
  \item We must first decide how to store and represent input and intermediate results.
    ...
    that is exchanged between processes. Instead % TODO
  ...
  \item Finally, in the context of tropical varieties it is desirable to consider the
    ...
    $\mathcal F$ in $\setR^n$ we want to find a subgroup of permutations $S \leq S_n$
    that acts trivially on $\mathcal F$ but produces non-trivial cone orbits.
\end{enumerate}
...
\begin{figure}[tbhp]
  ...
  \begin{tikzpicture}[node distance=1.4cm]
    ...
    \node [place] (unpList) [right of=init] {$uL$} edge [pre] (init);
    \node [transition] (split) [below of=unpList] {$sL$}
...
\Grass(k,n)$ of $V$ with a pure tensor in $\bigwedge^kV \cong K^N$ uniquely up to scalar
multiplication which -- by definition -- makes the corresponding embedding $\Grass(k,n)
\to \setP^{N-1}$, called the \emph{Plücker embedding}, injective. Hence we can regard
$\Grass(k,n)$ as a subset of this projective space. Finally, to equip $\Grass(k,n)$ with
the structure of a projective variety, we need this fact: For non-zero $w\in
\bigwedge^kK^n$ the rank of the $K$-linear map $f:K^n \to \bigwedge^{k+1}K^n, v \mapsto v
\wedge w$ is at least $n-k$ and equality holds if and only if $w$ is a pure tensor. Thus,
using the above embedding, $\Grass(k,n)$ consists entirely of pure tensors $w\in
\Grass(k,n)$ which all define maps $v\mapsto v\wedge w$ of rank at most $n-k$. This is
equivalent to all $(n-k+1) \times (n-k+1)$-minors of these maps vanishing which induces
...
\begin{example}
  As we clearly have $\Grass(0,n) \cong \Grass(n,n) \cong K^n$ and $\Grass(1,n) \cong
  \Grass(n-1, n) \cong \setP_K^{n-1}$, the first \enquote{non-trivial} Grassmannian is
  $\Grass(2,4)$. By the process described above one easily finds that $\Grass(2,4)$ is
  given by the ideal
  \[
    I_{2,4} := \ideal{ x_{i,j} \phi : 1 \leq i < j \leq 4 }
    \idealof K[x_{1,2}, \dots, x_{3,4}].
  \]
  for $\phi = x_{1,2}x_{3,4} - x_{1,3}x_{2,4} + x_{1,4}x_{2,3}$, but $\dim I_{2,4} = 5$,
  so it defines a hypersurface in $\setP^5$. It can be shown that in fact $\sqrt{I_{2,4}}
  = \ideal \phi$, so $\Grass(2,4) = V(\phi)$ as one would expect. Hence the polyhedral fan
  corresponding to the tropical variety $\mathcal G_{2,4}$ has maximal 5-dimensional cones
  living in $\setR^6$.
  % TODO
\end{example}
