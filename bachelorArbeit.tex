...
of Fraunhofer ITWM, in particular with Dr.~Mirko Rahn and Christian Reinbold.
Additionally, as a side effect we accelerated the development of \texttt{puiseux.lib}
\cite{puiseuxLib} lead by Santiago Laplagne on the one hand, which is used as a fallback
for method computing Puiseux expansions of roots of zero-dimensional ideals. On the other
hand during our work with \textsc{Singular} we were able to identify several bugs --
mainly memory leaks -- which were promptly fixed. Using our parallel algorithm we were
able to compute the tropical Grassmannian $\mathcal G_{3,8}$ modulo symmetry in its
entirety, which was not possible with legacy methods until now. Using this as benchmark,
we also found that our algorithm scales favorably to large processor counts.
...
\begin{table}[tbh]
  ...
  \begin{tabular}{r|r||r}
    ...
    \textbf{cores} & \textbf{one machine} & \textbf{two machines} \\ \hline
    1   & 792.806 &      -- \\
    2   & 389.855 & 375.859 \\
    4   & 190.589 & 191.739 \\
    8   &  99.254 &  96.990 \\
...
minimal and maximal running times observed for two cores on one machine respectively.
\todo{in progress}
...
\todo{obviously not complete yet}
