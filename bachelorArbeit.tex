...
\begin{definition}[Tropicalization and Initial Forms]
  ...
  residue field $\mathfrak K = R_\nu/\mathfrak m_{R_\nu}$. In other words, $\initial_{\vec
  w}(f)$ consists of all the terms $c_\alpha\cdot \vec x^\alpha$ of $f$ where
  $\nu(c_\alpha)+\vec w\cdot \alpha$ is minimal. Again, $\initial_{\vec w}(f)$ need not be
  a monomial.
  \label{def:initialId}
\end{definition}
